...
| ハンバーガーメニュー | ヘッダーは共通画像。開いた時に PC-Menu（1920×1000）/ SP-Menu（750×1400）の y≥120 部分を画像として重ねる。9 項目・閉じるボタンは当たり判定（項目はページ内の実在アンカーへの `a`、閉じるは `button`）。Escape・フォーカス復帰・背景スクロール抑止 | viewport 1920×1000 / 375×700@2x で開状態を撮影し、Menu ページの PDF と比較して 0.000% |
