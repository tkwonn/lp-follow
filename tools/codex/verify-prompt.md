...
- 正解画像: `build/img/{pc,sp}/<slice>.png`（`design/*.pdf` を pdftoppm 72dpi で描画。1pt = 1px）。
- 合格条件: 正解と Chromium の撮影が **差分画素 0 個（0.000%）**。PC は viewport 幅 1920・DPR 1、SP は viewport 幅 375・DPR 2。詳細は `CLAUDE.md` の「再現度の基準」と `thoughts/plans/2026-09-07_pixel-exact-implementation-plan.md` §2。
...
3. `build/codex/` に自分の Playwright スクリプトを書いて、`python3 -m http.server 4198 -d out` などで `out/` を配信し、次を確認する:
   ...
   - 幅 1920（DPR1）と 375（DPR2）で、対象区画の各切り出し要素の高さが `docs/slices.json` の高さと一致すること（SP は CSS px で半分）。
