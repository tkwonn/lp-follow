...
- 正解画像: `build/img/{pc,sp}/<slice>.png`（`design/*.pdf` を **pdftocairo** で描画。PC は 144dpi＝幅 3840px（1pt = 2px）、SP は 72dpi＝幅 750px（1pt = 1px）。2026-09-07 改定。pdftoppm は使わない）。
- 合格条件: 正解と Chromium の撮影（device px）が **差分画素 0 個（0.000%）**。PC は viewport 幅 1920・**DPR 2**（device 3840px）、SP は viewport 幅 375・DPR 2（device 750px）。DPR 1 は 0.000% の対象外（仕様）。詳細は `CLAUDE.md` の「再現度の基準」と `thoughts/plans/2026-09-07_pixel-exact-implementation-plan.md` §2。
...
3. `build/codex/` に自分の Playwright スクリプトを書いて、`python3 -m http.server 4198 -d out` などで `out/` を配信し、次を確認する:
   ...
   - 幅 1920（DPR2）と 375（DPR2）で、対象区画の各切り出し要素の CSS 高さが `docs/slices.json` の高さ（PC はそのまま、SP は半分）と一致し、device px では正解 PNG の実寸（PC 3840 幅、SP 750 幅）と一致すること。
   - PC の正解 PNG の細線（ヘッダーのハンバーガー 3 本線）が等幅であること、および実装の DPR 2 撮影でも等幅に写ること（旧条件で真ん中だけ太かった不具合の再発確認）。
